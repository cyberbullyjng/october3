...
import { 
  useListBlacklistedServers, 
  useBlacklistServer, 
  useUpdateBlacklistedServer, 
  useUnblacklistServer 
...
export default function BlacklistServers() {
  ...
  const { data, isLoading } = useListBlacklistedServers({
    page,
    limit: 20,
    ...(debouncedSearch && { search: debouncedSearch }),
  });

  const addMutation = useBlacklistServer();
  const updateMutation = useUpdateBlacklistedServer();
  const removeMutation = useUnblacklistServer();

  const handleAdd = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await addMutation.mutateAsync({
        data: { discordId, serverName, reason }
      });
      toast({ title: "Server blacklisted successfully" });
      setIsAddOpen(false);
      setDiscordId("");
      setServerName("");
      setReason("");
      queryClient.invalidateQueries({ queryKey: ["/api/blacklist/servers"] });
    } catch (err: any) {
      toast({ 
        variant: "destructive", 
        title: "Error", 
        description: err.message || "Failed to blacklist server" 
      });
    }
  };
  ...
  const handleRemove = async (id: number) => {
    if (!confirm("Are you sure you want to unblacklist this server?")) return;
    try {
      await removeMutation.mutateAsync({ id });
      toast({ title: "Server unblacklisted" });
      queryClient.invalidateQueries({ queryKey: ["/api/blacklist/servers"] });
    } catch (err: any) {
      toast({ 
        variant: "destructive", 
        title: "Error", 
        description: err.message || "Failed to unblacklist server" 
      });
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h1 className="text-2xl font-bold tracking-tight">Server Blacklist</h1>
        <button 
      ...
      <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="p-4 border-b border-border bg-muted/20">
          <div className="relative w-full sm:max-w-md">
            ...
            <input 
              type="text" 
              placeholder="Search by server name or Discord ID..." 
              value={search}
              ...
              {isLoading ? (
                ...
                data.data.map((server) => (
                  <tr key={server.id} className="hover:bg-muted/30 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-muted-foreground font-mono">{server.discordId}</div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-muted-foreground">{server.reason}</span>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-muted-foreground whitespace-nowrap">
                      {format(new Date(server.addedAt), "MMM d, yyyy")}
                    </td>
                    <td className="px-6 py-4 text-right whitespace-nowrap">
                      <button
                        onClick={() => {
                          setSelectedServer(server);
                          setReason(server.reason);
                          setIsEditOpen(true);
                        }}
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-16 text-center border-t border-border">
                    <ShieldBan className="w-12 h-12 mx-auto text-muted-foreground mb-4 opacity-50" />
                    <h3 className="text-lg font-medium text-foreground mb-1">No blacklisted servers</h3>
                    <p className="text-muted-foreground text-sm">The server blacklist is currently empty.</p>
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <div className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="px-6 py-4 border-b border-border flex justify-between items-center">
              <h2 className="text-lg font-semibold flex items-center">
                <ShieldBan className="w-5 h-5 mr-2 text-destructive" />
                Blacklist Server
              </h2>
            </div>
            <form onSubmit={handleAdd} className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea 
                  ...
                  placeholder="Why is this server being blacklisted?"
                />
              </div>
            ...
            <form onSubmit={handleEdit} className="p-6 space-y-4">
              <div>
                <p className="text-sm text-muted-foreground mb-4">
                  Updating reason for <span className="font-semibold text-foreground">{selectedServer.serverName}</span> ({selectedServer.discordId})
                </p>
                <label className="block text-sm font-medium text-foreground mb-1">Reason</label>
                <textarea 
                  required
